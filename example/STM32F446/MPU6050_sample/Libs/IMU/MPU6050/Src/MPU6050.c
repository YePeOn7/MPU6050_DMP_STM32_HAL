...
//			printf("mpu_set_dmp_state complete ......\r\n");
	...
	MPU6050_setZeroMotionDetectionThreshold(5); //set this value to make it very sensitive to movement, the higher the lesser sensitive
...
// setStartTime: 1 -> get startTime and dont perform calculation of Yaw Corrector Rate
void MPU6050_updateYawCorrectorRate(uint8_t isStable)
{
	static uint8_t first = 1;
	static uint32_t lastTime = 0;
	static float lastAngle = 0;
	static int32_t interval = 5000;
//	static lastSetStartTime = 0;

	if(first || !isStable)
	{
		lastTime = HAL_GetTick();
		lastAngle = MPU6050_Yaw;
		first = 0;
	}
	else
	{
		...
		{
			float delta = MPU6050_Yaw-lastAngle;

			if(delta > 180) delta -= 360;
			else if(delta < -180) delta += 360;

			MPU6050_YawCorrectorRate -= (delta) * 1000 / (HAL_GetTick() - lastTime);
			lastTime = HAL_GetTick();
			lastAngle = MPU6050_Yaw;
		}
	}
//	lastSetStartTime = setStartTime;
}
...
void MPU6050_updateAngleCorrector(void)
{
	...
	else
	{
		if(HAL_GetTick() - lastTime > interval)
		{
			float dt = (float)(HAL_GetTick()-lastTime) / 1000;
			MPU6050_PitchCorrector += MPU6050_PitchCorrectorRate * dt;
			MPU6050_RollCorrector += MPU6050_RollCorrectorRate * dt;
			MPU6050_YawCorrector += MPU6050_YawCorrectorRate * dt;

			if(MPU6050_YawCorrector > 180) MPU6050_YawCorrector -= 360;
			else if(MPU6050_YawCorrector < -180) MPU6050_YawCorrector += 360;

			if(MPU6050_PitchCorrector > 180) MPU6050_PitchCorrector -= 360;
			else if(MPU6050_PitchCorrector < -180) MPU6050_PitchCorrector += 360;

			if(MPU6050_RollCorrector > 180) MPU6050_RollCorrector -= 360;
			else if(MPU6050_RollCorrector < -180) MPU6050_RollCorrector += 360;
...
void MPU6050_GyroCalibrationV2(int loop)
{
	float average[3] = {0};
	short gyroOffset[3] = {0};
	/************** FIRST STEP CALIBRATION **********/
	for(int step = 0; step < 3; step++){
		memset(average,0, sizeof(average));
		MPU6050_getAllGyroOffset(gyroOffset);
		for(int i = 0;i < loop; i++){
			while(MPU6050_readDMP() != 0);
			for(int j = 0; j < 3; j++) average[j] += MPU6050_gyroRAW[j]*2;
			delay_ms(1);
		}
		for(int i = 0; i < 3; i++) {
			average[i] = average[i] / loop;
			gyroOffset[i] += i == 2? -(round(average[i])):round(average[i]);
		}
		MPU6050_setAllGyroOffset(gyroOffset);
		delay_ms(10);
	}
